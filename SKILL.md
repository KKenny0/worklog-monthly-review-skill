...
骨架数据中的 `category_distribution` 字段包含各类别（能力升级/结构变更/问题定位等）的数量分布，直接用于"工作量分布"表格。
骨架数据中的 `work_phases` 字段包含按周的工作阶段分析，可辅助撰写"本月概览"中的工作节奏判断。
...
- 每个项目最多保留 2-3 个核心主题，其余合并为一句"日常优化包括..."
- 代码变更数字只在净增 >300 行或净减 >200 行时保留，其余不写具体数字
- 配置调整、文档优化、<50 行变更的小修复一律合并，不单独列出
- 运维类工作（日志、监控、通知、CI/CD）合并为一句
- "本月概览"应包含工作节奏判断（如"上半月集中...下半月转向..."），可参考 `work_phases`

**章节结构：**

- **本月概览**：2-4 句话叙事
- **工作量分布**：表格，使用骨架中的 `category_distribution`
- **本月主线**：每个真实项目一行
- **[项目名]**：叙事段落（1-3 段），不是加粗标题列表
- **问题与风险**：从骨架的 `risks` 和未完成事项中提取，每条一行
- **下月衔接**：从骨架的 `next_actions` 中提取，每条一行
- **数据说明**：统计口径、误检测说明等
- 误检测项目（不在 `real_projects` 列表中的）不单独展开
- 总结总长度控制在 80-120 行以内
- **输出格式以 `reference/worklog-summary-template.md` 为准**
